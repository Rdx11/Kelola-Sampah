...
class ReportController extends Controller
{
    ...
    public function updateStatus(Request $request, $id): RedirectResponse
    {
        ...
        // Only increment or insert point if status is 'accepted'
        if ($request->status === 'accepted') {
            $user = $report->user;
            $userPoint = $user->userPoints()->first();

            if ($userPoint) {
            // Increment point if already exists
            $userPoint->increment('point', 10);
            } else {
            // Insert new point record if not exists
            $user->userPoints()->create(['point' => 10]);
            }
        }
